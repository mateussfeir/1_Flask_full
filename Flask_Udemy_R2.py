...
def chart():
    if request.method == 'POST':
        ...
        stock = session.get('stock', 'AAPL')  # Default to AAPL if session doesn't have a stock
    ...
    # Separate the information in the list
    ...
    plt.title(f"{stock}'s {'quarterly'} {'Net Income'}")

    # Save the chart to a buffer and embed it in the HTML response
    img = io.BytesIO()
